'use client';
import toggleBookmark from '@/app/api/actions/toggleBookmark.ts';
...
export default function Bookmark({ id, bookmarked }: { id: string; bookmarked: boolean }) {
  ...
  const handleClick = async () => {
    ...
    setIsLoading(true);
    try {
      const toggleBookmarkResponse = await toggleBookmark(id, isBookmarked);
      
      if (toggleBookmarkResponse) {
      ...
      >
        {!isBookmarked ? (
          <svg
            ...
            viewBox='0 0 384 512'
            className='w-5 h-6 text-yellow-800 z-50'
            fill='currentcolor'
          >
          ...
          <svg
            xmlns='http://www.w3.org/2000/svg'
            viewBox='0 0 384 512'
            className='w-5 h-6 text-yellow-800'
            fill='currentcolor'
          >
